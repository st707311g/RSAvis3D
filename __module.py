...
from __common import REGISTRATED_DESTINATION, SEGMENTATED_DESTINATION, tqdm
...
class VolumePath(object):
    ...
    def segmentated_volume_directory(self):
        dir_name, base_name = os.path.split(self.directory)
        destination = dir_name+'/'+self.segmentated_directory+'/'+base_name
        return destination
...
class VolumeLoader(object):
    ...
    def image_file_list(self):
        img_files = [os.path.join(self.volume_directory, f) for f in os.listdir(self.volume_directory)]
    ...
    def load(self) -> np.ndarray:
        ...
        logger.info(f'Loading {self.image_file_number} image files: {self.volume_directory}')
...
class VolumeSaver(object):
    ...
    def save(self, extension='jpg'):
        ...
        for i, img in enumerate(tqdm(self.np_volume)): #type: ignore
            image_file = os.path.join(self.destination_directory, f'img{str(i).zfill(4)}.{extension}')
...
class RSAvis3D(object):
    def __init__(self, np_volume:np.ndarray):
        super().__init__()
        self.__volume: Union[cp.ndarray, np.ndarray] = cp.array(np_volume) if is_cupy_available else np_volume
    ...
    def block_separator(self, overlapping=0, block_size=64, all_at_once = False):
        ...
        temp_volume = pad_function(self.__volume, overlapping, mode = 'symmetric')
        ...
        for zi in range(0, shape[0], block_size):
            for yi in range(0, shape[1], block_size):
                for xi in range(0, shape[2], block_size):
                    blocks.append(temp_volume[zi:zi+block_size+overlapping*2, 
    ...
    def make_sylinder_mask(self, radius: int):
        ...
        self.__mask = (x - shape[1]/2)**2 + (y - shape[2]/2)**2 < radius**2
        self.__mask: np.ndarray = np.repeat([self.__mask], shape[0], axis=0)

    def trim_with_mask(self, padding = 0):
        slices = []
        for axis in range(3):
            v = np.max(self.__mask, axis=tuple([i for i in range(3) if i != axis]))
            index = np.where(v)
            slices.append(slice(max(np.min(index)-padding, 0), min(np.max(index)+padding+1, self.__mask.shape[axis])))
    ...
    def apply_median3d(self, block_size=64,  median_kernel_size=1, all_at_once=False):
        logger.info(f'Median3D filter: {block_size=}, {median_kernel_size=}, {all_at_once=}')
        ...
        for blocks, indexes in i_block:
            if is_cupy_available:
                blocks = [cp_median_filter(b, size=median_kernel_size) for b in tqdm(blocks)]
            else:
    ...
    def detect_edge(self ,kernel_size=21, intensity_factor=1.):
        logger.info(f'Edge detection filter: {kernel_size=}, {intensity_factor=}')
